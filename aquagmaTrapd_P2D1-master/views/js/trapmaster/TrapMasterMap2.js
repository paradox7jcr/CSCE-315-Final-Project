var TrapMasterMap2 = function(game){
...
var trapCoord = new Array(2940);

var HEALTH = 0;
var INVENTORY;

TrapMasterMap2.prototype = {
	preload: function(){ 

		//Preloads map 1 tiles and map from the assets folders
	    this.game.load.tilemap('tilemap', 'assets/maps/map2.json', null, Phaser.Tilemap.TILED_JSON);
		...
		this.game.load.audio('TrapMasterSong2', 'assets/music/fur_elise.ogg');
	...
	create: function() {
		...
		trapperMusic2 = game.add.audio('TrapMasterSong2');
	 	trapperMusic2.loopFull();
		...
		this.game.world.setBounds(0,0,1920,1568);
		...
		this.game.stage.backgroundColor = "#556b2f";
		...
		this.currentTile = this.map.getTile(0, 0, this.mainLayer);
		this.currentTrapTile = this.map.getTile(0, 0, this.mainLayer);
		
		//Change the world size to match the size of this layer
		this.mainLayer.resizeWorld();
		
		/*//List of all tiles that the player collides with.
		this.map.setCollisionBetween(14, 19, true, 'Wall');
		this.map.setCollisionBetween(137, 138, true, 'Wall');
		this.map.setCollisionBetween(462, 464, true, 'Wall');
		this.map.setCollisionBetween(702, 705, true, 'Wall');
		this.map.setCollisionBetween(732, 735, true, 'Wall');
		this.map.setCollisionBetween(891, 892, true, 'Wall');*/
		
		//Makes the Tiled map into a collision based map.
		//this.game.physics.p2.convertTilemap(this.map, this.mainLayer);
	...
	canPlaceTrap: function() {
		if (
			this.currentTile.index==12||
			this.currentTile.index==13||
			this.currentTile.index==14||
			this.currentTile.index==20||
			this.currentTile.index==21||
			this.currentTile.index==22||
			this.currentTile.index==23||
			this.currentTile.index==24||
			this.currentTile.index==51||
			this.currentTile.index==52||
			this.currentTile.index==53||
			this.currentTile.index==54||
			this.currentTile.index==74||
			this.currentTile.index==75||
			this.currentTile.index==76||
			this.currentTile.index==77||
			this.currentTile.index==81||
			this.currentTile.index==82||
			this.currentTile.index==83||
			this.currentTile.index==84||
			this.currentTile.index==110||
			this.currentTile.index==111||
			this.currentTile.index==112||
			this.currentTile.index==140||
			this.currentTile.index==142||
			this.currentTile.index==170||
			this.currentTile.index==171||
			this.currentTile.index==172||
			this.currentTile.index==587||
			this.currentTile.index==643||
			this.currentTile.index==673||
			this.currentTile.index==796||
			this.currentTile.index==797||
			this.currentTile.index==825||
			this.currentTile.index==826||
			this.currentTile.index==855||
			this.currentTile.index==856||
			this.currentTile.index==857||
			this.currentTile.index==884||
			this.currentTile.index==885||
			this.currentTile.index==886||
			this.currentTile.index==888
			) return true;
		else
		{
			console.log('Can\'t place tile at: ' +this.currentTile.index);
			return false;
		}
	},
	...
	mouseClick: function() {
		...
		if(this.game.input.keyboard.isDown(Phaser.Keyboard.SHIFT)){
			if(this.canPlaceTrap()) {
				if(trapperStartGold < 1500 && this.currentTrapTile.index == 901){
					trapperStartGold += 75;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 1500 && this.currentTrapTile.index == 902){
					trapperStartGold += 10;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 1500 && this.currentTrapTile.index == 903){
					trapperStartGold += 50;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 1500 && this.currentTrapTile.index == 904){
					trapperStartGold += 25;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 1500 && this.currentTrapTile.index == 905){
					trapperStartGold += 50;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
			}
			...
			if((this.game.input.activePointer.worldY >= this.minimizeButton.y) && (this.game.input.activePointer.worldY <= this.minimizeButton.y + this.minimizeButton.height) && (this.game.input.activePointer.worldX >= this.minimizeButton.x) && (this.game.input.activePointer.worldX <= this.minimizeButton.x + this.minimizeButton.width)) {}
				...
				if(this.spikePlacing == true && trapperStartGold >= 75){
					if(this.canPlaceTrap() && this.currentTrapTile.index != 901){
						trapperStartGold -= 75;
					...
					if(this.canPlaceTrap() && this.currentTrapTile.index != 902){
						trapperStartGold -= 10;
					...
					if(this.canPlaceTrap() && this.currentTrapTile.index != 903){
						trapperStartGold -= 50;
					...
					if(this.canPlaceTrap() && this.currentTrapTile.index != 904){
						trapperStartGold -= 25;
					...
					if(this.canPlaceTrap() && this.currentTrapTile.index != 905){
						trapperStartGold -= 50;
	...
	mapDone: function() {
		var i = 0;
		var tiles = this.trapLayer.getTiles(0, 0, 1920, 1568);
		//var trapCoord = new Array(2940);
		var str;
		for(var y = 0; y < 1568; y+=32) {
		...
		var tileMapInfo = {mapNumber: 2, newMap: trapCoord};
		...
		if(runnerReadyFirst){
			this.game.state.start('SpectateMap2');
		}
		else{
	...
	update: function() {
		...
		if(this.toolbar.exists == true) {
			if(this.game.input.activePointer.worldY <= this.toolbar.y || this.game.input.activePointer.worldX <=this.toolbar.x || this.game.input.activePointer.worldX >= this.toolbar.x + this.toolbar.width) {
				this.marker.x = this.mainLayer.getTileX(this.game.input.activePointer.worldX) * 32;
				this.marker.y = this.mainLayer.getTileY(this.game.input.activePointer.worldY) * 32;
				this.currentTile = this.map.getTile(this.mainLayer.getTileX(this.marker.x), this.mainLayer.getTileY(this.marker.y), this.mainLayer);
			}
		}
		else {
			this.marker.x = this.mainLayer.getTileX(this.game.input.activePointer.worldX) * 32;
	    ...
	    //If the player presses the down arrow
			...
			if(this.game.camera.y < 1568-800)
			{
				this.game.camera.y += 5;
				this.selected.y +=5;
				this.goldText.y +=5;
		...
		//If the player presses the right arrow
			...
			if(this.game.camera.x < 1920-1200)
			{

var TrapMasterMap3 = function(game){
...
TrapMasterMap3.prototype = {
	preload: function(){ 
		...
		this.game.load.audio('TrapMasterSong3', 'assets/music/fur_elise.ogg');
	...
	create: function() {
		...
		trapperMusic3 = game.add.audio('TrapMasterSong3');
	 	trapperMusic3.loopFull();
		...
		this.map.addTilesetImage('trapTiles', 'trapTiles');

		
	    //Add both the background and ground layers. We won't be doing anything with the
	    //GroundLayer though
	    this.backgroundLayer 	= this.map.createLayer('background_tiles');
	    this.mainLayer 			= this.map.createLayer('collision_tiles');
	    this.trapLayer			= this.map.createLayer('trap_tiles');
	...
	canPlaceTrap: function() {
		if (
			this.currentTile.index==33||
			this.currentTile.index==212||
			this.currentTile.index==243
			) return true;
		else
		{
	...
	mouseClick: function() {
		...
		if(this.game.input.keyboard.isDown(Phaser.Keyboard.SHIFT)){
			if(this.canPlaceTrap()) {
				if(trapperStartGold < 2000 && this.currentTrapTile.index == 901){
					trapperStartGold += 75;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 2000 && this.currentTrapTile.index == 902){
					trapperStartGold += 10;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 2000 && this.currentTrapTile.index == 903){
					trapperStartGold += 50;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 2000 && this.currentTrapTile.index == 904){
					trapperStartGold += 25;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
				else if(trapperStartGold < 2000 && this.currentTrapTile.index == 905){
					trapperStartGold += 50;
					this.map.putTile(820, this.trapLayer.getTileX(this.marker.x), this.trapLayer.getTileY(this.marker.y), this.trapLayer);
				}
			}
	...
	mapDone: function() {
		...
		var tileMapInfo = {mapNumber: 3, newMap: trapCoord};
		...
		if(runnerReadyFirst){
			this.game.state.start('SpectateMap3');
		}
		else{
	...
	update: function() {
	    ...
	    //If the player presses the down arrow
			...
			if(this.game.camera.y < 1600-800)
			{
				this.game.camera.y += 5;
				this.selected.y +=5;
				this.goldText.y +=5;
		...
		//If the player presses the right arrow
			...
			if(this.game.camera.x < 1600-1200)
			{
